...
async function main() {
  ...
  await prisma.kbArticle.upsert({
    ...
    create: {
      ...
      categoryId: (await prisma.category.findFirstOrThrow({ where: { slug: 'plumbing' } })).id,
      content: '<h2>Overview</h2><p>Check flapper, chain, fill valve.</p>',
      difficulty: 'easy',
      timeEstimateMin: 30,
      toolsJson: [{ name: "Adjustable Wrench", purpose: "Loosen/tighten nuts", affiliate_url: "https://www.amazon.com/s?k=adjustable+wrench" }] as any,
      stepsJson: [
